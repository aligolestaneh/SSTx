...
class ActiveLearner:
    ...
    def __init__(self, model, query_strategy, datasets):
        """Initialize with a model, a query strategy, and datasets."""
        self.model = model
        self.query_strategy = query_strategy
        self.datasets = datasets

    def load_datasets(self):
        """Load datasets from the provided dictionary."""
        self.x_pool = self.datasets["x_pool"].copy()
        self.y_pool = self.datasets["y_pool"].copy()
        self.x_val = self.datasets["x_val"].copy()
        self.y_val = self.datasets["y_val"].copy()

    def fit(self, plot=False, **kwargs):
        """Train the model on the current training data."""
        losses, val_losses = self.model.fit(
            self.x_train, self.y_train, self.x_val, self.y_val, **kwargs
        )
        if plot:
    ...
    def score(self, **kwargs):
        ...
        return self.model.score(self.x_val, self.y_val, **kwargs)
    ...
    def learn(
        ...
        **kwargs,
        ...
        # Initial fit
        self.fit(save_best="train", **kwargs)
        if model_name:
