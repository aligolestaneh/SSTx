...
class Trajectory:
    ...
    def to_step_waypoints(self, dt: float) -> np.ndarray:
        """Convert this trajectory to a list of joint waypoints
        at each given time steps
        """
        # Data holder
        total_run_step = int(np.ceil(self.end_time / dt))
        waypoints = np.zeros((3, total_run_step, self.dof))

        for k in range(total_run_step):
            curr_t = k * dt
            waypoints[0, k, :] = self.position(curr_t)
            waypoints[1, k, :] = self.velocity(curr_t)
            waypoints[2, k, :] = self.acceleration(curr_t)
        return waypoints
